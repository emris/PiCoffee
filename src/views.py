...
def login():
    ...
    if form.validate_on_submit():
        user = db.session.query(Users).query.filter_by(uid=form.uid.data).first()
        if user.role > 0:
...
@login_required
def coffee():
    cu = db.session.query(Users).filter_by(id=current_user.id).first()
...
def brands(a):
    ...
    brands = db.session.query(CoffeeBrands).join(bmax, bmax.c.vermax == CoffeeBrands.version).filter(bmax.c.name_id == CoffeeBrands.name_id).all()
    adata = db.session.query(ActiveData).filter_by(id=1).first()
    if request.method == 'POST':
        if (a == "USE"):
            ...
            brand = db.session.query(CoffeeBrands).filter_by(id=request.form['id']).first()
            if brand.hide:
                brand.hide = False
            else:
                brand.hide = True
            ...
            if form.validate_on_submit():
                cn = db.session.query(CoffeeNames).filter_by(name=form.name.data).first()
                if cn:
                    brand = db.session.query(CoffeeBrands).filter_by(name_id=cn.id).order_by(desc(CoffeeBrands.version)).first()
...
def accounts(a):
    ...
    if (a == "list"):
        users = db.session.query(Users).filter(Users.id > 1).all()
        return render_template('accounts.html', users=users, title=title)
    return redirect('/coffee')
